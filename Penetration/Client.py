...
class ClientUDP(UDPTransferProtocol):
    ...
    def wait(self, socket=None, address=None, ack=0):
        if socket is None or address is None:
            super().wait(self.socket, address=(self.SERVER_IP, self.SERVER_PORT), ack=ack)
        else:
            super().wait(socket=socket, address=address, ack=ack)

    def close(self):
        self.socket.close()

c = ClientUDP()
c.send(mes=' This is client mes ...')
print('Client sended a mes -> ')
